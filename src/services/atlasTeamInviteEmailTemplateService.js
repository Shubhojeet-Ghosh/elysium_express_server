...
function generateTeamInviteEmail({
  ...
  const inviterLabel = inviterName?.trim() || "A team owner";
  const teamLabel = teamName?.trim() || "their team";
                ...
                <table width="100%" style="background:${COLORS.WHITE};max-width:600px;margin:40px auto 0 auto;padding:30px 0 30px 0;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                  <tr>
                    <td style="padding:32px 32px 16px 32px;text-align:left;">
                      <h2 style="margin:0 0 8px 0; color:${COLORS.DEEP_ONYX}; font-size:20px;">You're invited!</h2>
                      <p style="margin:0; color:${COLORS.DEEP_ONYX}; font-size:24px; font-weight:600;">
                        Join <span style="color:${COLORS.SERENE_PURPLE};">${teamLabel}</span> on
                        <span style="color:${COLORS.DEEP_ONYX};">Elysium</span><sup style="color:${COLORS.SERENE_PURPLE}; font-size:0.9em; vertical-align:super; line-height:0;">atlas</sup>
                      </p>
                      <p style="margin:24px 0 0 0; color:${COLORS.DEEP_ONYX}; font-size:15px; opacity:0.8;">
                        <b style="color:${COLORS.DEEP_ONYX};">${inviterLabel}</b> invited
                        <span style="color:${COLORS.SERENE_PURPLE}; font-weight:bold;">${inviteeEmail}</span>
                        to collaborate on their team.<br/>
                        Click below to review and respond to this invitation.<br/>
                        <b style="color:${COLORS.DEEP_ONYX};">This link expires in ${expiresInDays} days.</b>
                      </p>
                      ...
                      <div style="margin-bottom:18px;text-align:center;">
                        ...
                        <a href="${inviteLink}" style="color:${COLORS.SERENE_PURPLE};word-break:break-all;font-size:13px;">
                          ${inviteLink}
                        </a>
                      </div>
...
module.exports = { generateTeamInviteEmail };
